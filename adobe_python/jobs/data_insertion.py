    ...
    if isinstance(r, dict):
        ...
        if re.search("SUCCESS", run):
            ...
            try:
                ...
                class_files.Files({}).writeFile({"file":f"{directory_log}/{r.get('time')}.json", "content":json.dumps({"request":r.get('data'), "response":response}, sort_keys=False, indent=4, default=str)})  
